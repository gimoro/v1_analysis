...
rdss <- dir("/Users/gimoro/Desktop/v1_analysis_thesis/FACS_data/data/test fith scRNAseq", "*.fcs", 
...
titration<-titration[c(3,2,1)]

sampleNames(titration)<-c("pos","neg","rock")
pData(titration)$name<-c("pos","neg","rock")
...
sqrcut <- matrix(c(60000,70000,80000,90000,100000,120000,130000,120000,100000,90000,80000,70000,60000,
                   60000,105000,120000,125000,125000,110000,90000,45000,40000,38000,40000,45000,60000),ncol=2,nrow=13)
...
plot<-ggcyto(titration, aes(x = FSC.A, y = SSC.A)) + 
  geom_hex(bins = 100) + 
  facet_wrap(~factor(name,levels = c("pos","neg","rock")),ncol = 3) +
  geom_gate(beads) + 
...
ggcyto(filt_titration, aes(x = FSC.A, y = SSC.A)) + 
  geom_hex(bins = 100) + 
  facet_wrap(~factor(name, levels = c("pos","neg","rock"))) +
  geom_gate(beads) + 
  theme_bw() +
  facs_rock_roi_theme_multiple_panels()
...
ggcyto(filt_titration, aes(x = "APC-A", fill = name,color=name)) + 
  ...
  facet_wrap(~factor(name, levels = c("pos","neg","rock")),ncol=1,switch = "y")+
  theme_bw() +
...
ggplot(plottable,aes(x=factor(L1,levels=c("pos","neg","rock")),y=value))+
