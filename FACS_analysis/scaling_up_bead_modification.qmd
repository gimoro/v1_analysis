---
title: "scaling_up_bead_modification"
...
fdir<-"/Users/gimoro/Desktop/v1_analysis_thesis/FACS_data/data/BD beads scaled up"
rdss <- dir(fdir, "*.fcs", 
...
titration<-titration[c(3,1,6,2,5,4)]

sampleNames(titration)<-c("pos","neg","unmod","small_v","12_epis","10_epis")
pData(titration)$name<-c("pos","neg","unmod","small_v","12_epis","10_epis")
...
sqrcut <- matrix(c(50000,60000,70000,80000,90000,
                   110000,120000,110000,90000,80000,
                   70000,60000,50000,70000,115000,130000,
                   135000,135000,120000,100000,55000,50000,
                   48000,50000,55000,70000),ncol=2,nrow=13)
...
plot<-ggcyto(titration, aes(x = FSC.A, y = SSC.A)) + 
  geom_hex(bins = 100) + 
  facet_wrap(~factor(name,levels = c("pos","neg","unmod","small_v","12_epis","10_epis")),ncol = 3) +
  geom_gate(beads) + 
...
ggcyto(filt_titration, aes(x = FSC.A, y = SSC.A)) + 
  geom_hex(bins = 100) + 
  facet_wrap(~factor(name, levels = c("pos","neg","unmod","small_v","12_epis","10_epis"))) +
  geom_gate(beads) + 
  theme_bw() +
  facs_rock_roi_theme_multiple_panels()
...
ggplot(plottable,aes(x=factor(L1,levels=c("pos","neg","unmod",
                                          "small_v","12_epis","10_epis")),y=value))+
