...
```{r}
fdir<-"/Users/gimoro/Desktop/v1_analysis_thesis/FACS_data/data/Test third scRNAseq"
rdss <- dir(fdir, "*.fcs", 
...
titration<-titration[c(3,2,4,1)]

sampleNames(titration)<-c("pos","neg","unmod","rock")
pData(titration)$name<-c("pos","neg","unmod","rock")
...
sqrcut <- matrix(c(60000,70000,80000,90000,100000,
                   120000,130000,120000,100000,90000,80000,70000,60000,
                   70000,115000,130000,135000,135000,120000,100000,55000,
                   50000,48000,50000,55000,70000),ncol=2,nrow=13)
...
plot<-ggcyto(titration, aes(x = FSC.A, y = SSC.A)) + 
  geom_hex(bins = 100) + 
  facet_wrap(~factor(name,levels = c("pos","neg","unmod","rock")),ncol = 3) +
  geom_gate(beads) + 
...
ggcyto(filt_titration, aes(x = FSC.A, y = SSC.A)) + 
  geom_hex(bins = 100) + 
  facet_wrap(~factor(name, levels = c("pos","neg","unmod","rock"))) +
  geom_gate(beads) + 
  theme_bw() +
  facs_rock_roi_theme_multiple_panels()
...
ggcyto(filt_titration, aes(x = "APC-A", fill = name,color=name)) + 
  ...
  facet_wrap(~factor(name, levels = c("pos","neg",
                                      "unmod","rock")),ncol=1,switch = "y")+
  theme_bw() +
...
ggplot(plottable,aes(x=factor(L1,levels=c("pos","neg","unmod","rock")),y=value))+
...
ggplot(plottable,aes(x=factor(L1,levels=c("pos","neg",
                                          "unmod","rock")),y=value))+
   scale_y_flowjo_biexp()+
