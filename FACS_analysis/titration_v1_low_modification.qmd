---
title: "titration_v1_low_modification"
...
```{r}
fpath<-"/Users/gimoro/Desktop/v1_analysis_thesis/FACS_data/data/titration_low_modification"
rdss <- dir(fpath, "*.fcs", 
...
titration<-titration[c(6,5,7,3,4,2,1)]

sampleNames(titration)<-c("pos","neg","unmod","100","50","10","1")
pData(titration)$name<-c("pos","neg","unmod","100","50","10","1")
...
sqrcut <- matrix(c(40000,50000,60000,70000,80000,100000,
                   ...
                   60000,105000,120000,125000,125000,110000,90000,
                   45000,40000,38000,40000,45000,60000),ncol=2,nrow=13)
...
ggcyto(filt_titration, aes(x = "APC-A", fill = name,color=name)) + 
  ...
  facet_wrap(~factor(name, levels = c("pos","neg","unmod","100","50","10","1")),ncol=1,switch = "y")+
  theme_bw() +
...
ggplot(plottable,aes(x=factor(L1,levels=c("pos","neg","unmod","100","50","10","1")),y=value))+
...
ggplot(summarized,aes(x=factor(sample,levels=c("pos","neg","unmod","100","50","10","1")),y=mean))+
   scale_y_flowjo_biexp()+
...
for_corr_neg<-plottable[which(plottable$L1 %in% c("100","10","50","1","neg")),]
...
for_corr<-plottable[which(plottable$L1 %in% c("100","10","50","1")),]
